...
// Сжатие изображения (уменьшаем до 800px по большей стороне, качество 0.8)
...
async function loadUserPhoto(userId) {
    try {
        ...
        if (docSnap.exists()) {
            ...
            console.log('📸 Фото загружено, есть?', data.photoBase64 ? 'да' : 'нет');
            return data.photoBase64 || null;
        } else {
            console.log('📸 Документ не найден, создаю');
...
window.saveDisplayName = async function() {
    ...
    try {
        ...
        console.log('✅ Ник сохранён:', newName);
