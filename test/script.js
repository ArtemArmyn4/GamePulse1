...
onAuthStateChanged(auth, async (user) => {
    ...
    renderAllGalleries();
    renderFavorites();
    renderProfileFavorites(); // Обновляем галерею в профиле
    const favCountEl = document.getElementById('profileFavoritesCount');
    if (favCountEl) favCountEl.textContent = favorites.length;
});
...
window.openGame = function(gameId) {
    ...
    let html = `
        <img src="${game.image}" alt="${game[`title_${currentLang}`]}">
        <div class="game-meta">${game.year} | ${game.developer}</div>
        <div class="game-desc">${game[`desc_${currentLang}`]}</div>
        <div class="reqs">
            <div>
                <h4>${lang[currentLang].minReqs || 'Минимальные требования'}</h4>
                <ul>${game.min.map(i => `<li>${i}</li>`).join('')}</ul>
            </div>
            <div>
                <h4>${lang[currentLang].recReqs || 'Рекомендуемые'}</h4>
                <ul>${game.rec.map(i => `<li>${i}</li>`).join('')}</ul>
            </div>
        </div>
    `;
    ...
    if (game.playtime_main && game.playtime_completion) {
        html += `
            <div class="modal-playtime">
                <h4>${lang[currentLang].playtime_title || '⏱️ Время прохождения'}</h4>
    ...
    if (game.metacritic || game.steam || game.ign) {
        html += `
            <div class="modal-ratings">
                <h4>${lang[currentLang].ratings_title || '📊 Оценки'}</h4>
    ...
    if (game.build) {
        html += `
            <div class="modal-build">
                <h4>${lang[currentLang].build_title || '🧩 Собери идеальный ПК под игру'}</h4>
                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
    ...
    html += `
        <div class="doctor-block">
            <h4>${lang[currentLang].doctorTitle || '🤖 Гейм-Доктор'}</h4>
            <p>${getDoctorVerdict(gameId)}</p>
        </div>
    `;
